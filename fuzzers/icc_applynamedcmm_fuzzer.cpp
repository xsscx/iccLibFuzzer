 ...
 *
 * Fuzzer for IccApplyNamedCmm - Named Color CMM transformation workflow.
 * Honors AST Query pattern for proper profile loading and attachment.
 *
 * Exercises:
 * - CIccNamedColorCmm profile loading and validation
 * - Named color to pixel transformations (icApplyNamed2Pixel)
 * - Pixel to pixel transformations (icApplyPixel2Pixel)
 * - Named color to named color transformations (icApplyNamed2Named)
 * - Multiple rendering intents (0-3, +modifiers)
 * - Interpolation modes (linear, tetrahedral)
 ...
 *
 * Design Pattern:
 * Follows iccApplyNamedCmm.cpp architecture:
 * 1. Parse fuzzer input for configuration
 * 2. Create CIccNamedColorCmm with proper source/dest spaces
 * 3. Add profile(s) with AddXform() using hint manager
 * 4. Call Begin() to initialize CMM chain
 * 5. Apply transformations based on detected interface
 * 6. Exercise encoding conversion (ToInternal/FromInternal)
 */
