# Robust sanitizers for CI step summaries and logs
# Intended to be SOURCED by workflows (do not `exit` from this file).
#
# Goals:
# - Remove/control dangerous control characters (CR, NUL, etc.)
# - Escape HTML entities to avoid XSS in GITHUB_STEP_SUMMARY
# - Provide both single-line (`sanitize_line`) and multi-line (`sanitize_print`)
#   helpers with reasonable length limits and trimming rules
# - Helpers to sanitize refs/filenames used in concurrency/group names
#
# Usage (example):
#   # in workflow step (after obtaining the trusted sanitize.sh)
#   # shellcheck disable=SC1090
#   source /path/to/newest-sanitizer.sh
#
# Notes:
# - This file is safe to source from CI jobs. It does not call `exit` or set
#   `set -e` so sourcing won't change the caller's shell options.
# - Keep the functions POSIX/bourne-compatible where practical but using bash
#   expansions is acceptable because workflows use `shell: bash`.
#
