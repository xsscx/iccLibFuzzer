...
#
#
# Run: /bin/bash -c "$(curl -fsSL https://raw.githubusercontent.com/InternationalColorConsortium/iccDEV/refs/heads/research/contrib/HelperScripts/pr247.sh)"
#
#
###############################################################
...
echo ""
           ...
           output="$(iccFromXml sRGB_D65_colorimetric_61477.xml sRGB_D65_colorimetric_61477.icc 2>&1)"
           echo "$output" | grep -E "undefined" || true
           git show --stat --pretty=format:"Commit: %H%nAuthor: %an%nDate: %ad%n" HEAD | grep Commit
           git branch
           date
           cd ..
           set -e
           echo "========= Stop undefined-behavior Test ... PR247 =========="
echo ""
echo "======================================================"
echo "===================  ISSUE STOP   ===================="
echo "======================================================"
