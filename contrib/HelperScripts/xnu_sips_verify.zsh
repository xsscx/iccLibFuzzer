...
# Intent: Run sips on macOS for *.icc and Report
#
#
#
#
...
#
#
# Run: /usr/bin/env zsh <(curl -fsSL https://raw.githubusercontent.com/InternationalColorConsortium/iccDEV/refs/heads/research/contrib/HelperScripts/xnu_sips_verify.zsh)
#
#
#
#
...
find . -type f -name "*.icc" | while IFS= read -r icc_file; do
  ...
  # Create the output file name based on the base name and place it in the output directory
  output_file="${output_dir}/${base_name}-sips.txt"
...
# Final report
report_file="${output_dir}/sips_verification_report.txt"
